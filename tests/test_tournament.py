...
class Test10_FinishTournament(UnitTest):
    ...
    def test_003_finish_16_seeding(self):
        ...
        user3 = self.user([tj] * 11 + [tsa, tj, tj, ts, gs] + [tmf] * 8 + [gs] + [tmf] * 4 + [gs, tmf, tmf, tmf, tf])
        user4 = self.user([tj] * 10 + [tsa, tj, tj, ts, gs] + [tmf] * 8 + [gs] + [tmf] * 4 + [gs, tmf, tmf, tmf, tf])
        user5 = self.user([tj] * 9 + [tsa, tj, tj, ts, gs] + [tmf] * 8 + [gs] + [tmf] * 4 + [tmf, tmf, tmf, tf])
        user6 = self.user([tj] * 8 + [tsa, tj, tj, ts, gs] + [tmf] * 8 + [gs] + [tmf] * 4 + [tmf, tmf, tmf, tf])
        user7 = self.user([tj] * 7 + [tsa, tj, tj, ts, gs] + [tmf] * 8 + [gs] + [tmf] * 4 + [tmf, tmf, tmf, tf])
        user8 = self.user([tj] * 6 + [tsa, tj, tj, ts, gs] + [tmf] * 8 + [tmf] * 4 + [tmf, tmf, tmf, tf])
        user9 = self.user([tj] * 5 + [tsa, tj, tj, ts, gs] + [tmf] * 8 + [gs] + [tmf] * 4 + [tmf, tmf, tmf, tf])
        user10 = self.user([tj] * 4 + [tsa, tj, tj, ts, gs] + [tmf] * 8 + [tmf] * 4 + [tmf, tmf, tmf, tf])
        user11 = self.user([tj] * 3 + [tsa, tj, tj, ts, gs] + [tmf] * 8 + [tmf] * 4 + [tmf, tmf, tmf, tf])
        user12 = self.user([tj] * 2 + [tsa, tj, tj, ts, gs] + [tmf] * 8 + [tmf] * 4 + [tmf, tmf, tmf, tf])
        user13 = self.user([tj] * 1 + [tsa, tj, tj, ts, gs] + [tmf] * 8 + [tmf] * 4 + [tmf, tmf, tmf, tf])
        user14 = self.user([tsa, tj, tj, ts, gs] + [tmf] * 8 + [tmf] * 4 + [tmf, tmf, tmf, tf])
        user15 = self.user([tj, ts, gs] + [tmf] * 8 + [tmf] * 4 + [tmf, tmf, tmf, tf])
        user16 = self.user([ts, gs] + [tmf] * 8 + [tmf] * 4 + [tmf, tmf, tmf, tf])
